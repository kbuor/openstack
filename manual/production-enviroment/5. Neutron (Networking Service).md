...
```
```shell
cat <<'EOF' > /usr/bin/neutron-api-wsgi
#!/usr/bin/python3
import eventlet
eventlet.monkey_patch()

from neutron import server
application = server.initialize_wsgi_app()
EOF
```
```shell
chmod +x /usr/bin/neutron-api-wsgi
chown root:root /usr/bin/neutron-api-wsgi
```
```shell
...
<VirtualHost *:9696>
    ...
    WSGIScriptAlias / /usr/bin/neutron-api-wsgi
